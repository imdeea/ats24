<?
$title = 'Rotary Die Cutting Services | Tapes | Fabrics | Films | Elastomers';
$meta_descr = 'Advanced Technology Supply, Inc provides bespoke, precision rotary die-cutting services for a wide range of materials. MilItary-spec. Get a FREE quote!';
$schema = "<script type='application/ld+json'>
{
  \"@context\": \"http://www.schema.org\",
  \"@type\": \"service\",
  \"brand\": \"Advanced Technology Supply, Inc\",
  \"url\": \"https://ats24.com/rotary-die-cutting.php\",
  \"name\": \"Rotary Die Cutting Services\",
  \"image\": \"https://ats24.com/images/rotary-die-cutting.png\",
  \"description\": \"Advanced Technology Supply, Inc provides bespoke, precision rotary die-cutting services for a wide range of materials. MilItary-spec\n\"
}
 </script>";
include 'inc_header.php'; ?>

	<section class="section-intro pb0">
		<div class="container">
			<div class="row">
				<div class="col-md-5 offset-md-1 col-md-push-7">
					<img class="blog-post-image" src="/images/rotary-die-cutting.png" alt="Rotary Die Cutting Machinery (Slicing Tape, Films, Fabrics)" title="Rotary Die Cutting Machinery (Slicing Tape, Films, Fabrics)" width="450" height="338">
				</div>
				<div class="col-md-6 col-md-pull-5">
					<h1 class=" mb20">Rotary Die Cutting Services</h1>
					<p class="mb30">
						Successfully meeting the needs of multiple industries, including the military, our <b>rotary die cutting services</b> can be applied to a wide assortment of materials, including foil, fiber, plastic, and rubber. Our rotary die cutting machinery produces pressure sensitive tapes, fabrics, films, and thin elastomers "kiss cut" to a client's particular shape or pattern. We can also use our rotary die cutting operations to cut a variety of tape, such as single and double coated, transfer, duct, and masking. We can <b>slice tape</b> measuring up to 100 yards long and between 0.5 to 25 millimeters thick - meeting tight precision tolerances of (+/-) .010 of an inch.
						<br><br>
						Rotary die cutting offers a number of unique advantages. Rotary dies can cut the material down to but not through the liner. Additionally, the liner may be underscored for easy removal from the die cut part. This equipment can also be used to apply pressure sensitive adhesive to elastomers, films, and fabrics, as well as introduce release liners in continuous lengths. Finally, and most advantageously, rotary die cutting allows us to "join" different materials in order to take advantage of the properties of each material.
						<br><br>
						Our die cut products include air filters, conveyor belts, magnets, and rubber goods. Our ISO compliant rotary die cutting operations meet military specifications. We handle small to medium production runs in short lead times and can even help you select the appropriate material for your application.  <a href="contact.php">Contact</a> Advanced Technology today for more information about our top-notch rotary die cutting capabilities.
					</p>
					<div class="col-sm-12 col-md-12 text-right mt30">
						<a href="request-quote.php" class="btn btn-icon btn-a" style="text-transform: uppercase; font-size: 20px;"><i class="fa fa-file-invoice-dollar" style="font-size: 25px"></i> Request a Quote</a>
					</div>
				</div>

			</div>
		</div>
	</section>

	<section class="section">
		<div class="container">
			<div class="row">
				<div class="col-sm-12">
					<h2 class="title-border">Portfolio</h2>
				</div>
			</div>
			<div class="row col-p0 max_height sm_max_height">
				<div class="col-sm-12 col-md-4">
					<a href="custom-rewinding-ptfe-tape-rolls.php">
						<div class="box-services-d bg-img" style="background-image: url(images/custom-slitting-rewinding01.jpg)">
							<div class="bg-overlay"></div>
							<div class="row col-p0">
								<div class="col-sm-12">
									<h3 class="title-medium br-bottom">Custom Rewinding of PTFE Tape Rolls for the Aircraft Industry</h3>
								</div>
							</div>
						</div>
					</a>
				</div>
				<div class="col-sm-12 col-md-4">
					<a href="custom-slitting-rewinding-ptfe-tape-rolls.php">
						<div class="box-services-d bg-img" style="background-image: url(images/overhaul-industry01.jpg)">
							<div class="bg-overlay"></div>
							<div class="row col-p0">
								<div class="col-sm-12">
									<h3 class="title-medium br-bottom">Custom Slitting & Rewinding of PTFE Tape Rolls for the Aircraft Repair and Overhaul Industry</h3>
								</div>
							</div>
						</div>
					</a>
				</div>
				<div class="col-sm-12 col-md-4">
					<a href="custom-die-cutting-polyester-masking-tape.php">
						<div class="box-services-d bg-img"  style="background-image: url(images/custom-die-cutting01.jpg)">
							<div class="bg-overlay"></div>
							<div class="row col-p0">
								<div class="col-sm-12">
									<h3 class="title-medium br-bottom">Custom Die Cutting of Polyester Masking Tape for the Electronics Industry  </h3>
								</div>
							</div>
						</div>
					</a>
				</div>
			</div>
		</div>
	</section>

	<section class=" section-bg mt0 mb0" style="background: #fff;">
		<div class="container">
			<div class="row">
				<div class="col-md-12">

				<article class="blog-post-content">
						<h2 class="mb20">Rotary Die Cutting Capabilities</h2>
				</article>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<table>
						 <tbody>
							<tr>
							   <td class="width30p"><strong>General Capabilities</strong></td>
							   <td class="width35p">
									Contract<br/>
									Large Format <br/>
									Offshore
								</td>
							   <td class="width30p">
									On-Demand<br/>
									Short Run
								</td>
							</tr>
							<tr>
							   <td><strong>Industry Focus</strong></td>
							   <td class="noborder">
									Aerospace<br>
									Automotive<br>
									Chemical<br>
									Construction<br>
									Electrical<br>
									Electronics<br>
									Food Processing<br>
									Machinery
							  </td>
							   <td class="noborder">
									Military<br>
									Medical<br>
									Optical<br>
									Packaging<br>
									Pharmaceutical<br>
									Printing<br>
									Railroad<br>
									Recreational
							   </td>
							</tr>
							<tr>
							   <td><strong>Die Cutting Method</strong></td>
							   <td class="noborder">
									Rotary
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>Materials</strong></td>
							   <td class="noborder">
									Adhesives<br>
									Aluminum Foil<br>
									Cloths<br>
									Copper Foil<br>
									Cork<br>
									Fiber<br>
									Films<br>
									Felt<br>
									Foam<br>
									Glass Cloth<br>
									Metalized Paper<br>
									Paper<br>
									Plastics<br>
									Rubber<br>
									Sponge
							  </td>
							   <td class="noborder">
									Tape
									<ul>
										<li>Transfer </li>
										<li>Double Coated </li>
										<li>DC </li>
										<li>Cloth </li>
										<li>Duct </li>
										<li>Filament </li>
										<li>Film </li>
										<li>Masking </li>
										<li>Foil </li>
										<li>Masking </li>
										<li>Medical </li>
										<li>Paper </li>
										<li>Single Coated </li>
									</ul>
									Teflon<br>
									Ultem<br>
									VHB<br>
									Vinyl
							   </td>
							</tr>
							<tr>
							   <td><strong>Die Cut Products</strong></td>
							   <td class="noborder">
									Air Filters<br>
									Belts<br>
									Conveyor Belts<br>
									Electrical Insulation<br>
									Electronic Parts<br>
									EMI/RFI Shielding<br>
									Film<br>
									Gaskets<br>
									Insulation
							  </td>
							   <td class="noborder">
									Labels<br>
									Magnets<br>
									Mats<br>
									Release Paper<br>
									Rubber Goods<br>
									Rubber Parts<br>
									Tags<br>
									Tapes
							   </td>
							</tr><tr>
							   <td><strong>Additional Services Provided</strong></td>
							   <td class="noborder">
									Design<br>
									Film Laminating (limited width)<br>
									Perforating<br>
									Slitting
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>Production Volume</strong></td>
							   <td class="noborder">
									Small to Medium Runs
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>Lead Times</strong></td>
							   <td class="noborder">
									Same Day (if inventory in stock)
							  </td>
							   <td class="noborder">
									up to 6 weeks
							   </td>
							</tr>

							<tr>
							   <td><strong>Width</strong></td>
							   <td class="noborder">
									Up to 8 in.
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>Length</strong></td>
							   <td class="noborder">
									Up to 100 yards
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>Thickness</strong></td>
							   <td class="noborder">
									0.5 to 25 mil
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>Diameter</strong></td>
							   <td class="noborder">
									Up to 12 in.
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>Tolerance (+/-)</strong></td>
							   <td class="noborder">
									.010 in.
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>Industry Standards</strong></td>
							   <td class="noborder">
									ISO 9001:2008
									Mil Spec<br>
									Qualify for Veteran Owned Small Business
							  </td>
							   <td class="noborder">
							   </td>
							</tr>
							<tr>
							   <td><strong>File Formats</strong></td>
							   <td class="noborder">
									Images
							  </td>
							   <td class="noborder">
									PDF
							   </td>
							</tr>
						 </tbody>
					  </table>
				</div>
			</div>
		</div>
	</section>

	<style>
	table{ width: 100%; }
	td.width65p{
		width: 65%;
	}
	td.width35p {
		width: 35%;
	}
	td{
		border-left: 1px solid #ccc;
		border-right: 1px solid #ccc;
		border-collapse: collapse;
		padding: 3px 8px;
		vertical-align: top;
	}

	td.width30p {
		width: 30%;
	}

	tr {
		border: 1px solid #ccc;
		border-collapse: collapse;
	}

	</style>


<? include 'inc_footer.php'; ?>
